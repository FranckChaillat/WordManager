...
module PathFinding =
    ...
    let branchHead = 
        let rec apply acc (paths: Path list)(nodeToMatch)(newNode : Node)(relationName) = 
            match paths with
            | path :: tail ->
    ...
    let branchTail initPaths = 
        let rec apply acc (paths : Path list)(nodeToMatch)(newNode : Node)(relationName : string) =
            match paths with
            | ((_ :: t) as path) :: tail ->
                        match List.rev path with
                        | (h :: _) as rev when h == nodeToMatch -> 
                            let edge = {name = relationName; nextNode = newNode; direction = LeftToRight}
                            let updated = {rev.Head with outputNodes = edge :: rev.Head.outputNodes}
                            let appended = List.ofSeq (Seq.take (path.Length - 1) path) 
                                           @ [updated; newNode]
                            apply (appended :: acc) tail nodeToMatch newNode relationName
                        | _ -> 
                            apply (acc) tail nodeToMatch newNode relationName
    ...
    let rec getGraphFullPaths (acc: Path list) (nodeLst : Node list) : Path list =
        ...
        | h :: t -> let nxtNodes = h.outputNodes
                    let paths = nxtNodes 
                                 |> List.fold(fun a x -> let newPaths = (branchTail acc h x.nextNode x.name) |? []
                                                                         @ ((branchHead acc x.nextNode h x.name) |? [])
